...
BaseType_t intTaskSwitchPending;
...
//-----------------------------------------------------------
	...
	//
	void portTaskSwitchContext(BaseType_t xSwitchRequired)
	{
		intTaskSwitchPending |= xSwitchRequired;					
																
		if(NOT_NESTING() && intTaskSwitchPending)					
		{															
			register BaseType_t uxSavedPmicCtrlReg; 				
...
static void prvSetupTimerInterrupt( void )
{
	...
	tc_set_resolution(&TCC0, sysclk_get_per_hz() / configTICK_RATE_HZ);
	tc_write_period(&TCC0, tc_get_resolution(&TCC0) / configTICK_RATE_HZ);
	...
	tc_set_overflow_interrupt_level( &TCC0, TC_OVFINTLVL_LO_gc);	//low interrupt priority
#elif configKERNEL_INTERRUPT_PRIORITY == 1
	tc_set_overflow_interrupt_level( &TCC0, TC_OVFINTLVL_MED_gc);	//medium interrupt priority
#elif configKERNEL_INTERRUPT_PRIORITY == 2
	tc_set_overflow_interrupt_level( &TCC0, TC_OVFINTLVL_HI_gc);	//high interrupt priority
#endif
}
