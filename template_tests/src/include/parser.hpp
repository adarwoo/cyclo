...
public:
   ...
   const char *get_error(uint_least8_t &position) { position = this->distance; return err_; }
    ...
    */
   bool parse(
      const etl::string_view &buffer,
      const InteractHandler &interact_handler = [] (Interact) {}
      )
   {
      bool retval = false;
