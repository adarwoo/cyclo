...
#define portSAVE_SP() \
    asm volatile( \
        "lds	r26, pxCurrentTCB		;load X with address of SP  \n\t" \
...
#define portEXIT_CRITICAL() \
    asm volatile( \
        "pop	r31	      	;the saved pmic-ctrl register value 	\n\t" \
...
// Higher interrupts are still active and not locked out.
//
static inline BaseType_t _portSetInterruptMaskFromIsr(void)
{
    BaseType_t r = PMIC.CTRL;
    PMIC.CTRL &= ~PMIC_BITS;
    return r;
}
...
//
#if configMAX_SYSCALL_INTERRUPT_PRIORITY > 0
    extern BaseType_t intTaskSwitchPending;
