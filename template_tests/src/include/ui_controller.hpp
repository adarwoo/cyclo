...
inline void call_next() { controller_process_next = true; }
...
struct walkman {
  auto operator()() const noexcept {
    ...
    auto is_running = [] (UIModel& m) { return m.get_state() == UIModel::program_state_t::running; };
    auto is_stopped = [] (UIModel& m) { return m.get_state() == UIModel::program_state_t::stopped; };
    ...
    return make_transition_table(
        * "select"_s + sml::on_entry<_> / [] { call_next(); }
        ...
        , "do_play"_s + event<push> / [] (UIModel &m) { 
           fx::publish(msg::StartProgram{true});
           m.set_state(UIModel::program_state_t::running); } = "do_pause"_s
        
        , "do_pause"_s + sml::on_entry<_> / [] (UIView &v) { v.draw_walkman(1); }
        , "do_pause"_s + event<push> / [] (UIModel &m) { 
           fx::publish(msg::StopProgram{}); 
           m.set_state(UIModel::program_state_t::paused); } = "do_play_or_stop"_s
        , "do_pause"_s + event<up> [is_running]

        , "do_play_or_stop"_s + sml::on_entry<_> / [] (UIView &v) { v.draw_walkman(1); }
        , "do_play_or_stop"_s + event<push> / [] (UIModel &m) {
        ...
        , "do_stop"_s + event<push> / [] (UIModel &m) {
           fx::publish(msg::StopProgram{});
           m.set_state(UIModel::program_state_t::stopped); } = "do_play"_s
    );
  }
};
  ...
  auto operator()() const noexcept {
    ...
    return make_transition_table(
        * "setup_on_min"_s + event<up> [on_minutes_top]
        , "setup_on_min"_s + event<up> / [] (UIModel &m, UIView &v) { 
           ++m.on_min;
           v.manual_program_draw_digit(UIView::next_highlight, 0, 0); }
        , "setup_on_min"_s + event<down> [on_minutes_bottom]
        , "setup_on_min"_s + event<down> / [] (UIModel &m, UIView &v) { 
           --m.on_min;
           v.manual_program_draw_digit(UIView::next_highlight, 0, 0); }
        , "setup_on_min"_s + event<push> / [] (UIView &v) { 
           v.manual_program_draw_digit(UIView::back_to_normal, 0, 0); 
           v.manual_program_draw_digit(UIView::first_highlight, 0, 1); } = "setup_on_sec"_s
           
        , "setup_on_sec"_s + event<up> [on_seconds_top]          
        , "setup_on_sec"_s + event<up> / [] (UIModel &m, UIView &v) { 
           ++m.on_sec; 
           v.manual_program_draw_digit(UIView::next_highlight, 0, 1); }
        , "setup_on_sec"_s + event<down> [on_seconds_bottom]
        , "setup_on_sec"_s + event<down> / [] (UIModel &m, UIView &v) { 
           --m.on_sec; 
           v.manual_program_draw_digit(UIView::next_highlight, 0, 1); }
        , "setup_on_sec"_s + event<push> / [] (UIView &v) { 
           v.manual_program_draw_digit(UIView::back_to_normal, 0, 1);
           v.manual_program_draw_digit(UIView::first_highlight, 1, 0); } = "setup_off_min"_s
           
        , "setup_off_min"_s + event<up> [off_minutes_top]
        , "setup_off_min"_s + event<up> / [] (UIModel &m, UIView &v) { 
           ++m.off_min; 
           v.manual_program_draw_digit(UIView::next_highlight, 1, 0); }
        , "setup_off_min"_s + event<down> [off_minutes_bottom]
        , "setup_off_min"_s + event<down> / [] (UIModel &m, UIView &v) { 
           --m.off_min; 
           v.manual_program_draw_digit(UIView::next_highlight, 1, 0); }
        , "setup_off_min"_s + event<push> / [] (UIView &v) { 
           v.manual_program_draw_digit(UIView::back_to_normal, 1, 0); 
           v.manual_program_draw_digit(UIView::first_highlight, 1, 1); } = "setup_off_sec"_s
           
        , "setup_off_sec"_s + event<up> [off_seconds_top]
        , "setup_off_sec"_s + event<up> / [] (UIModel &m, UIView &v) { 
           ++m.off_sec; 
           v.manual_program_draw_digit(UIView::next_highlight, 1, 1); }
        , "setup_off_sec"_s + event<down> [off_seconds_bottom]
        , "setup_off_sec"_s + event<down> / [] (UIModel &m, UIView &v) { 
           --m.off_sec; 
           v.manual_program_draw_digit(UIView::next_highlight, 1, 1); }
    );
  }
...
struct mode_manual {
  auto operator()() const noexcept {
    ...
    auto is_running = [] (UIModel& m) { return m.get_state() == UIModel::program_state_t::running; };
    auto is_paused  = [] (UIModel& m) { return m.get_state() == UIModel::program_state_t::paused;  };
    auto is_stopped = [] (UIModel& m) { return m.get_state() == UIModel::program_state_t::stopped; };
    ...
    return make_transition_table(
        ...
        , state<program_selection> + event<push> / [] (UIModel &m) {
           m.load_command(); } = state<walkman>

        , "counter"_s + sml::on_entry<_> / [] (UIView &v) { 
           v.draw_cursor(2); }
        , "counter"_s + sml::on_exit<_> / [] (UIView &v) {
           v.erase_cursor(2); }
        , "counter"_s + event<down> = "contact"_s
        , "counter"_s + event<up> = state<walkman>
        , "counter"_s + event<push> / [] (UIModel &m, UIView &v) { 
           m.reset_counter(); 
           v.draw_counter(); }

        , "contact"_s + sml::on_entry<_> / [] (UIView &v) { 
           v.draw_cursor(3); }
        , "contact"_s + sml::on_exit<_> / [] (UIView &v) { 
           v.erase_cursor(3); }
        , "contact"_s + event<up> [ is_paused ] = "counter"_s
        , "contact"_s + event<up> = state<walkman>
        , "contact"_s + event<push> / [] (UIModel &m) { 
           m.flip_contact(); }

        , state<program_selection> + event<down> = state<walkman>
        , state<program_selection> + event<push> / [] (UIView &v) {
           v.draw_prog(false); } = state<walkman>
