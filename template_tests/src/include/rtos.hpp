...
namespace rtos
{
   ...
   public:
      ...
      template<typename T> bool send(T &what, tick_t tickToWait = tick::infinite )
         { return (xMessageBufferSend(handle, sizeof(T), &what, tickToWait) == pdPASS) ? true : false; }

      template<typename T> bool receive(T &what, tick_t tickToWait = tick::infinite )
         { return (xMessageBufferReceive(handle, sizeof(T), &what, tickToWait) == pdPASS) ? true : false; }

      template<typename T> bool send_from_isr(T &what)
         { return (xMessageBufferSendFromISR(handle, sizeof(T), &what, NULL) == pdPASS) ? true : false; }

      template<typename T> bool receive_from_isr(T &what)
